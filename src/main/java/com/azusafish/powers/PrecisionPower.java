package com.azusafish.powers;

import com.azusafish.orbs.SparkOrb;
import com.badlogic.gdx.graphics.Texture;
import com.badlogic.gdx.graphics.g2d.TextureAtlas;
import com.megacrit.cardcrawl.core.AbstractCreature;
import com.megacrit.cardcrawl.core.CardCrawlGame;
import com.megacrit.cardcrawl.helpers.ImageMaster;
import com.megacrit.cardcrawl.localization.PowerStrings;
import com.megacrit.cardcrawl.powers.AbstractPower;

public class PrecisionPower extends AbstractPower {
    public static final String POWER_ID = "AzusaFish:Precision";
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);
    public static final String NAME = powerStrings.NAME;
    public static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;

    public PrecisionPower(AbstractCreature owner, int amount) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        if (this.amount >= 10) {
            this.amount = 10;
        }
        this.type = PowerType.BUFF;
        
        // 临时的占位图标，请确保有对应的 png
        String unpath = "images/powers/precision.png"; 
        Texture img = ImageMaster.loadImage(unpath);
        // 如果没有图片，可以用 basegame 的某个图片代替，这里先尝试加载
        // 防止崩溃，可以catch或者使用region
        this.region128 = new TextureAtlas.AtlasRegion(img, 0, 0, 84, 84);
        this.region48 = new TextureAtlas.AtlasRegion(img, 0, 0, 32, 32);

        updateDescription();
    }

    @Override
    public void stackPower(int stackAmount) {
        this.fontScale = 8.0F;
        this.amount += stackAmount;
        if (this.amount > 10) {
            this.amount = 10;
        }
        updateDescription();
    }

    @Override
    public void updateDescription() {
        this.description = DESCRIPTIONS[0] + (this.amount * 10) + DESCRIPTIONS[1];
    }
}
